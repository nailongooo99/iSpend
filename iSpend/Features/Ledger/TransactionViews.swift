...
struct TransactionDetailView: View {
    ...
    var body: some View {
        List {
            Section { VStack(spacing: 12) { CategoryIcon(category: transaction.category, size: 58); AmountText(amount: transaction.amount, currency: transaction.currency, type: transaction.type, font: .largeTitle.bold()); Text(transaction.category?.pathDisplayName ?? transaction.type.title).foregroundStyle(.secondary) }.frame(maxWidth: .infinity).padding() }
...
struct TransactionSearchView: View {
    ...
    var body: some View { List { Picker("类型", selection: $type) { Text("全部").tag(nil as TransactionType?); ForEach(TransactionType.allCases) { transactionType in Text(transactionType.title).tag(Optional(transactionType)) } }.pickerStyle(.segmented); if results.isEmpty { ContentUnavailableView.search(text: query) } else { ForEach(results) { transaction in NavigationLink { TransactionDetailView(transaction: transaction) } label: { TransactionRow(transaction: transaction) } } } }.navigationTitle("搜索").searchable(text: $query, prompt: "金额、备注、分类、账户、标签") }
